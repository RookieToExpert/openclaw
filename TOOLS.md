...
* Volcano Job / PodGroup 查询和诊断：见 `2.3 查询任务`
...
rayctl job get job <job-name-or-pod-name-or-uid> [...]
...
rayctl -k /root/D/<vc-name> job get job <job-name>
```
...
`rayctl job get job` 支持传 job 名、pod 名或 UID。
`rayctl job get pg` 支持传 PodGroup 名或 UID。
`rayctl job check` 用于检查 Job 为什么起不来。
...
适用场景：`rayctl job check` / PodGroup 显示 `NotEnoughResources`，但用户认为 vcluster 总量应该有资源。

先用 rayctl 定位任务、vcluster、namespace、PodGroup 和 inspect Pod：

```bash
export KUBECONFIG=/root/kubeconfig
rayctl job get job <job-name-or-pod-name>
rayctl job check <job-name>
```
